...
static AOM_INLINE void compute_global_motion_for_ref_frame(
  ...
  collect_frame(src_width, src_height);

  //  collect_compute_global_motion_for_ref_frame(
  //      src_width, src_height, src_stride, bit_depth, segment_map_w,
  //      segment_map_h, downsample_level, num_refinements);
  ...
  for (TransformationType model = FIRST_GLOBAL_TRANS_TYPE;
    ...
    // --------------------------------------------------
    //        CALCULO RAPIDO DE ERRO NÃO SEGMENTADO
    // --------------------------------------------------

    //    start = clock();
    //
    //    int64_t pre_ref_frame_error = william_non_segmented_frame_error(
    //        is_cur_buf_hbd(xd), xd->bd, ref_buf[frame]->y_buffer,
    //        ref_buf[frame]->y_stride, cpi->source->y_buffer, src_stride,
    //        src_width, src_height);
    //
    //    gwmc_stats.time_non_segmented_frame_error =
    //        ((double)(clock() - start) * 1000000.0) / CLOCKS_PER_SEC;
    ...
    if (!aom_compute_global_motion(
            ...
            downsample_level, motion_models, RANSAC_NUM_MOTIONS,
            &mem_alloc_failed, &num_correspondences)) {
      if (mem_alloc_failed) {
    ...
    time_compute_global_motion(((double)(clock() - start) * 1000000.0) /
                               CLOCKS_PER_SEC);
    ...
    for (int i = 0; i < RANSAC_NUM_MOTIONS; ++i) {
      ...
      //      collect_affine_params(motion_models->params[0],
      //      motion_models->params[1],
      //                            motion_models->params[2],
      //                            motion_models->params[3],
      //                            motion_models->params[4],
      //                            motion_models->params[5],
      //                            tmp_wm_params.alpha, tmp_wm_params.beta,
      //                            tmp_wm_params.gamma, tmp_wm_params.delta);
      ...
      int seg_count = av1_compute_feature_segmentation_map(
      ...
      time_segmented_frame_error(((double)(clock() - start) * 1000000.0) /
                                 CLOCKS_PER_SEC);

      if (ref_frame_error == 0) continue;

      // --------------------------------------------------
      //       CALCULO RAPIDO DE ERRO SEGMENTADO WARPED
      // --------------------------------------------------

      start = clock();

      int64_t fast_error = william_fast_warp_error_segmented(
          is_cur_buf_hbd(xd), tmp_wm_params.wmmat, ref_buf[frame]->y_buffer,
          cpi->source->y_buffer, src_stride, ref_buf[frame]->y_crop_width,
          ref_buf[frame]->y_crop_height, segment_map, segment_map_w);

      time_fast_warp_error_segmented(((double)(clock() - start) * 1000000.0) /
                                     CLOCKS_PER_SEC);
      ...
      double input[8] = {
        src_width,                              // 'src_width'
        cpi->source->corners->num_corners,      // 'num_corners'
        motion_models->params[0],               // 'params_0'
        motion_models->params[1],               // 'params_1'
        seg_count,                              // 'seg_count'
        ref_frame_error,                        // 'ref_frame_error
        fast_error,                             // 'fast_error'
        (double)motion_models[i].num_inliers /  // 'inliers_rate'
            (double)num_correspondences
      };

      int skip = predict_should_skip(input);
      skipped(skip);

      time_prediction(((double)(clock() - start) * 1000000.0) / CLOCKS_PER_SEC);
      ...
      time_refine_integerized_param(((double)(clock() - start) * 1000000.0) /
                                    CLOCKS_PER_SEC);

      //      collect_error(ref_frame_error, fast_error, pre_ref_frame_error);
...
void av1_compute_gm_for_valid_ref_frames(
  ...
  //  collect_cpi(cpi->oxcf.rc_cfg.cq_level);

  //  collect_cm(cm->cur_frame->ref_count, cm->cur_frame->order_hint,
  //             cm->cur_frame->showable_frame,
  //             cm->ref_frame_map[frame]->ref_count,
  //             cm->ref_frame_map[frame]->order_hint,
  //             cm->ref_frame_map[frame]->showable_frame);
...
static AOM_INLINE void compute_global_motion_for_references(
  ...
  for (int frame = 0; frame < num_ref_frames; frame++) {
    collect_init();
    //    collect_compute_global_motion_for_references(
    //        num_ref_frames, reference_frame[frame].distance, frame);
