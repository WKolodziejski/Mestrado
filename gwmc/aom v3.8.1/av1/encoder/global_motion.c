...
int64_t av1_refine_integerized_param(
  ...
  fprintf(stderr, "V_best_error:%ld\n", best_error);

  if (best_error > selection_threshold) {
//    collect_warp_error(best_error);
    return INT64_MAX;
  }
...
int av1_compute_feature_segmentation_map(uint8_t *segment_map, int width,
  ...
  fprintf(stderr, "V_seg_count:%d\n", seg_count);

//  collect_av1_compute_feature_segmentation_map(seg_count);
...
// Aplica a transformação em um bloco do frame
static AOM_INLINE void fast_warp_segment(const int32_t *mat,
                                         const uint8_t *const ref,
                                         uint8_t *const tmp, int width,
                                         int height, int stride, int s_col,
                                         int s_row, int e_col, int e_row) {
  for (int i = s_row; i < s_row + e_row; ++i) {
    for (int j = s_col; j < s_col + e_col; ++j) {
      // Calculate transformed coordinates
      const int64_t x =
          (int64_t)mat[2] * j + (int64_t)mat[3] * i + (int64_t)mat[0];
      const int64_t y =
          (int64_t)mat[4] * j + (int64_t)mat[5] * i + (int64_t)mat[1];

      int32_t ix4 = (int32_t)(x >> WARPEDMODEL_PREC_BITS);
      int32_t iy4 = (int32_t)(y >> WARPEDMODEL_PREC_BITS);

      // Check if coordinates are within bounds
      if (ix4 >= 0 && ix4 < stride && iy4 >= 0 && iy4 < height) {
        // Copy pixel value from source to destination
        tmp[i * width + j] = ref[iy4 * stride + ix4];
      } else {
        // If out of bounds, assign a default value or handle it as needed
        // For simplicity, let's assume default value as 0
        tmp[i * width + j] = 0;
      }
    }
  }
...
static AOM_INLINE void highbd_fast_warp_segment(const int32_t *mat,
                                                const uint16_t *const ref,
                                                uint16_t *const tmp, int width,
                                                int height, int stride,
                                                int s_col, int s_row, int e_col,
                                                int e_row) {
  for (int i = s_row; i < s_row + e_row; ++i) {
    for (int j = s_col; j < s_col + e_col; ++j) {
      // Calculate transformed coordinates
...
static int64_t fast_warp_error_segmented(
    const int32_t *mat, const uint8_t *const ref, const uint8_t *const dst,
    int dst_stride, int p_width, int p_height, const uint8_t *segment_map,
    int segment_map_stride) {
  //  image(ref, p_width, p_height, dst_stride, "ref_seg.png");
  //  image(dst, p_width, p_height, dst_stride, "dst_seg.png");

  uint8_t *tmp = (uint8_t *)calloc(p_width * p_height, sizeof(uint8_t));
  ...
  for (int i = 0; i < p_height; i += WARP_ERROR_BLOCK) {
    for (int j = 0; j < p_width; j += WARP_ERROR_BLOCK) {
      int seg_x = j >> WARP_ERROR_BLOCK_LOG;
      int seg_y = i >> WARP_ERROR_BLOCK_LOG;
      // Only compute the error if this block contains inliers from the
      // motion model
      if (!segment_map[seg_y * segment_map_stride + seg_x]) continue;
      ...
      fast_warp_segment(mat, ref, tmp, p_width, p_height, dst_stride, j, i,
                        patch_w, patch_h);

      if (patch_w == WARP_ERROR_BLOCK && patch_h == WARP_ERROR_BLOCK) {
        sum_error += aom_sad32x32(tmp + j + i * p_width, p_width,
                                  dst + j + i * dst_stride, dst_stride);
      } else {
        sum_error +=
            generic_sad(tmp + j + i * p_width, p_width,
                        dst + j + i * dst_stride, dst_stride, patch_w, patch_h);
      }
    }
  }

  //  image(tmp, p_width, p_height, p_width, "tmp_seg.png");

  free(tmp);
  return sum_error;
}

static int64_t highbd_fast_warp_error_segmented(
    const int32_t *mat, const uint16_t *const ref, const uint16_t *const dst,
    int dst_stride, int p_width, int p_height, const uint8_t *segment_map,
    int segment_map_stride) {
  //  image(ref, p_width, p_height, dst_stride, "ref_seg.png");
  //  image(dst, p_width, p_height, dst_stride, "dst_seg.png");

  uint16_t *tmp = (uint16_t *)calloc(p_width * p_height, sizeof(uint16_t));
  ...
  for (int i = 0; i < p_height; i += WARP_ERROR_BLOCK) {
    for (int j = 0; j < p_width; j += WARP_ERROR_BLOCK) {
      ...
      highbd_fast_warp_segment(mat, ref, tmp, p_width, p_height, dst_stride, j,
                               i, patch_w, patch_h);

      if (patch_w == WARP_ERROR_BLOCK && patch_h == WARP_ERROR_BLOCK) {
        sum_error += aom_highbd_sad32x32(
            CONVERT_TO_BYTEPTR(tmp + j + i * p_width), p_width,
            CONVERT_TO_BYTEPTR(dst + j + i * dst_stride), dst_stride);
      } else {
        sum_error += generic_sad_highbd(tmp + j + i * p_width, p_width,
                                        dst + j + i * dst_stride, dst_stride,
                                        patch_w, patch_h);
      }
    }
...
int64_t william_fast_warp_error_segmented(int use_hbd, const int32_t *mat,
                                         const uint8_t *ref, const uint8_t *dst,
...
#if CONFIG_AV1_HIGHBITDEPTH
  if (use_hbd)
    return highbd_fast_warp_error_segmented(
        mat, CONVERT_TO_SHORTPTR(ref), CONVERT_TO_SHORTPTR(dst),
        dst_stride, p_width, p_height, segment_map, segment_map_stride);
#endif
  (void)use_hbd;
  return fast_warp_error_segmented(mat, ref, dst, dst_stride, p_width, p_height,
                                   segment_map, segment_map_stride);
}

static int64_t non_segmented_frame_error(const uint8_t *const ref,
                                         int ref_stride,
...
static int64_t highbd_non_segmented_frame_error(const uint16_t *const ref,
                                                int ref_stride,
...
int64_t william_non_segmented_frame_error(int use_hbd, int bd,
                                          const uint8_t *ref, int ref_stride,
                                          uint8_t *dst, int dst_stride,
                                          int p_width, int p_height) {
#if CONFIG_AV1_HIGHBITDEPTH
  if (use_hbd) {
    return highbd_non_segmented_frame_error(
        CONVERT_TO_SHORTPTR(ref), ref_stride, CONVERT_TO_SHORTPTR(dst),
        dst_stride, p_width, p_height, bd);
  }
#endif
  (void)use_hbd;
  (void)bd;
  return non_segmented_frame_error(ref, ref_stride, dst, dst_stride, p_width,
                                   p_height);
}
