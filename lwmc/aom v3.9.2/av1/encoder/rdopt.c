...
void collect_features(struct features_interp_filters *features,
  ...
  features->mbmi__interp_filters__as_filters__x_filter =
      interp_filter(mbmi->interp_filters.as_filters.x_filter);
  ...
  if (xd->left_mbmi) {
    ...
    features->xd__left_mbmi__interp_filters__as_filters__y_filter =
        interp_filter(xd->left_mbmi->interp_filters.as_filters.y_filter);
    features->xd__left_mbmi__interp_filters__as_filters__x_filter =
        interp_filter(xd->left_mbmi->interp_filters.as_filters.x_filter);
  ...
  if (xd->above_mbmi) {
    ...
    features->xd__above_mbmi__interp_filters__as_filters__y_filter =
        interp_filter(xd->above_mbmi->interp_filters.as_filters.y_filter);
    features->xd__above_mbmi__interp_filters__as_filters__x_filter =
        interp_filter(xd->above_mbmi->interp_filters.as_filters.x_filter);
  ...
  // rd_stats_y
  features->rd_stats_y__rate = rd_stats_y->rate;
  features->rd_stats_y__zero_rate = rd_stats_y->zero_rate;
  features->rd_stats_y__dist = rd_stats_y->dist;
  features->rd_stats_y__rdcost = rd_stats_y->rdcost;
  features->rd_stats_y__sse = rd_stats_y->sse;
  features->rd_stats_y__skip_txfm = rd_stats_y->skip_txfm;

  // rd_stats_uv
  features->rd_stats_uv__rate = rd_stats_uv->rate;
  features->rd_stats_uv__zero_rate = rd_stats_uv->zero_rate;
  features->rd_stats_uv__dist = rd_stats_uv->dist;
  features->rd_stats_uv__rdcost = rd_stats_uv->rdcost;
  features->rd_stats_uv__sse = rd_stats_uv->sse;
  features->rd_stats_uv__skip_txfm = rd_stats_uv->skip_txfm;
  ...
  // tile_data
  features->tile_data__abs_sum_level = tile_data->abs_sum_level;
  features->tile_data__allow_update_cdf = tile_data->allow_update_cdf;

  // tile_data->tile_info

  features->tile_data__tile_info__mi_row_start =
      tile_data->tile_info.mi_row_start;
  features->tile_data__tile_info__mi_row_end = tile_data->tile_info.mi_row_end;
  features->tile_data__tile_info__mi_col_start =
      tile_data->tile_info.mi_col_start;
  features->tile_data__tile_info__mi_col_end = tile_data->tile_info.mi_col_end;
  features->tile_data__tile_info__tile_row = tile_data->tile_info.tile_row;
  features->tile_data__tile_info__tile_col = tile_data->tile_info.tile_col;

  // tile_data->firstpass_top_mv
  features->tile_data__firstpass_top_mv__row = tile_data->firstpass_top_mv.row;
  features->tile_data__firstpass_top_mv__col = tile_data->firstpass_top_mv.col;

  // args
  features->args__ref_frame_cost = args->ref_frame_cost;
  features->args__single_comp_cost = args->single_comp_cost;
  features->args__skip_ifs = args->skip_ifs;
  features->args__start_mv_cnt = args->start_mv_cnt;
  features->args__interp_filter_stats_idx = args->interp_filter_stats_idx;
  features->args__wedge_index = args->wedge_index;
  features->args__wedge_sign = args->wedge_sign;
  features->args__diffwtd_index = args->diffwtd_index;
  features->args__best_pred_sse = args->best_pred_sse;
}
  ...
  for (int mode_index = mode_index_start; mode_index <= mode_index_end;
    ...
    *mbmi = base_mbmi;
...
#if WILLIAM_COLLECT_FEATURES
    struct features_interp_filters m_features;
    collect_init(&m_features);
#endif  // WILLIAM_COLLECT_FEATURES
    ...
    if (mbmi->motion_mode == SIMPLE_TRANSLATION && !is_interintra_mode) {
      ...
      start_timing(cpi, warped_causal_tool_time);
#endif
...
#if WILLIAM_COLLECT_FEATURES
        m_features.bsize = block_size(bsize);
        m_features.ref_best_rd = ref_best_rd;
        m_features.ref_skip_rd = *ref_skip_rd;
        m_features.best_est_rd = *best_est_rd;
        m_features.do_tx_search = do_tx_search;
        m_features.rate_mv = *rate_mv;
        m_features.num_planes = num_planes;
        m_features.is_comp_pred = is_comp_pred;
        m_features.rate2_nocoeff = rate2_nocoeff;
        m_features.interintra_allowed = interintra_allowed;
        m_features.txfm_rd_gate_level = txfm_rd_gate_level;
        m_features.update_type = frame_update_type(update_type);

        collect_features(&m_features, x, xd, mbmi, rd_stats, rd_stats_y,
                         rd_stats_uv, cpi, tile_data, args);
#endif  // WILLIAM_COLLECT_FEATURES

#if WILLIAM_PREDICT_SKIP
#if CONFIG_COLLECT_COMPONENT_TIMING
        start_timing(cpi, warped_causal_pred_time);
#endif

        int skip = will_discard(
            rd_stats->rdcost,
            *best_est_rd,
            cpi->rd.RDMULT,
            rd_stats->dist,
            prediction_mode(mbmi->mode),
            rd_stats->rate,
            *rate_mv,
            rd_stats_y->rate,
            cpi->ppi->twopass.firstpass_info.total_stats.MVrv,
            ...
            *cpi->ppi->tpl_sb_rdmult_scaling_factors,
            mbmi->skip_txfm,
            interintra_mode(mbmi->interintra_mode),
            x->rdmult,
            cpi->ppi->p_rc.rolling_actual_bits
            );

#if CONFIG_COLLECT_COMPONENT_TIMING
        end_timing(cpi, warped_causal_pred_time);
#endif

        if (skip) {
#if CONFIG_COLLECT_COMPONENT_TIMING
          end_timing(cpi, warped_causal_tool_time);
#endif
          continue;
        }
...
#endif
#if WILLIAM_COLLECT_FEATURES
        // william: foi pior; descartou
        m_features.discard = 1;
        collect_finalize(&m_features);
#endif
