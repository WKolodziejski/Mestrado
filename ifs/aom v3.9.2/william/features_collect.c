...
static char *interp_filters[] = { "REGULAR", "SMOOTH", "SHARP", "BILINEAR",
                                  "SHARP2" };
...
void collect_init(struct features_interp_filters *features) {
  features->filter_idx = -1;
  features->bsize = "";
  features->skip_pred = -1;
  features->num_planes = -1;
  features->tmp_rs = -1;
  features->min_rd = -1;
  features->switchable_rate = -1;
  features->rd = -1;
  features->last_best__as_filters__x_filter = -1;
  features->last_best__as_filters__y_filter = -1;
  features->discard = -1;
  ...
  features->mbmi__interp_filters__as_filters__y_filter = -1;
  features->mbmi__interp_filters__as_filters__x_filter = -1;
  ...
  features->xd__left_mbmi__interp_filters__as_filters__y_filter = -1;
  features->xd__left_mbmi__interp_filters__as_filters__x_filter = -1;
  ...
  features->xd__above_mbmi__interp_filters__as_filters__y_filter = -1;
  features->xd__above_mbmi__interp_filters__as_filters__x_filter = -1;
  ...
  features->rd_stats_luma__rate = -1;
  features->rd_stats_luma__zero_rate = -1;
  features->rd_stats_luma__dist = -1;
  features->rd_stats_luma__rdcost = -1;
  features->rd_stats_luma__sse = -1;
  features->rd_stats_luma__skip_txfm = -1;
...
static void collect_finalize_prediction(struct features_interp_filters *features) {
  FILE *file = fopen(filename, "a");
  fprintf(file, "%d,", features->discard);
  fprintf(file, "%d", features->prediction);
  fprintf(file, "\n");
  fclose(file);
}
...
void collect_finalize(struct features_interp_filters *features) {
  ...
  if (features->xd__left_available == 0) return;
  if (features->xd__up_available == 0) return;

  if (features->discard == 1) {
    if (lcg_rand16(&seed) % 200 > 0) return;
  } else {
    if (lcg_rand16(&seed) % 2 > 0) return;
  }

#if WILLIAM_COLLECT_FEATURES && WILLIAM_PREDICT_FEATURES
  collect_finalize_prediction(features);
  return;
#endif

  FILE *file = fopen(filename, "a");

fprintf(file, "%d,", features->filter_idx);
fprintf(file, "%s,", features->bsize);
//fprintf(file, "%d,", features->skip_pred);
//fprintf(file, "%d,", features->num_planes);
fprintf(file, "%d,", features->tmp_rs);
fprintf(file, "%lld,", features->min_rd);
fprintf(file, "%d,", features->switchable_rate);
fprintf(file, "%lld,", features->rd);
//fprintf(file, "%d,", features->last_best__as_filters__x_filter);
//fprintf(file, "%d,", features->last_best__as_filters__y_filter);
fprintf(file, "%d,", features->discard);
...
//fprintf(file, "%d,", features->mbmi__interp_filters__as_filters__y_filter);
fprintf(file, "%d,", features->mbmi__interp_filters__as_filters__x_filter);
...
fprintf(file, "%d,", features->xd__left_mbmi__interp_filters__as_filters__y_filter);
fprintf(file, "%d,", features->xd__left_mbmi__interp_filters__as_filters__x_filter);
...
fprintf(file, "%d,", features->xd__above_mbmi__interp_filters__as_filters__y_filter);
fprintf(file, "%d,", features->xd__above_mbmi__interp_filters__as_filters__x_filter);
...
//fprintf(file, "%d,", features->rd_stats_luma__rate);
//fprintf(file, "%d,", features->rd_stats_luma__zero_rate);
//fprintf(file, "%lld,", features->rd_stats_luma__dist);
//fprintf(file, "%lld,", features->rd_stats_luma__rdcost);
//fprintf(file, "%lld,", features->rd_stats_luma__sse);
//fprintf(file, "%d,", features->rd_stats_luma__skip_txfm);
...
#if WILLIAM_COLLECT_FEATURES && WILLIAM_PREDICT_FEATURES
  fprintf(file, "discard,prediction");
#else
  fprintf(file, "filter_idx,bsize,tmp_rs,min_rd,switchable_rate,rd,discard,x__rb,x__cnt_zeromv,xd__is_chroma_ref,xd__up_available,xd__left_available,xd__bd,mbmi__mode,mbmi__compound_idx,mbmi__interp_filters__as_filters__x_filter,xd__left_mbmi__bsize,xd__left_mbmi__mode,xd__left_mbmi__num_proj_ref,xd__left_mbmi__skip_txfm,xd__left_mbmi__compound_idx,xd__left_mbmi__interp_filters__as_filters__y_filter,xd__left_mbmi__interp_filters__as_filters__x_filter,xd__above_mbmi__bsize,xd__above_mbmi__mode,xd__above_mbmi__skip_txfm,xd__above_mbmi__compound_idx,xd__above_mbmi__interp_filters__as_filters__y_filter,xd__above_mbmi__interp_filters__as_filters__x_filter,cpi__tpl_rdmult_scaling_factors,cpi__ppi__internal_altref_allowed,cpi__ppi__twopass__kf_group_error_left,cpi__ppi__twopass__rolling_arf_group_actual_bits,cpi__ppi__twopass__sr_update_lag,cpi__ppi__p_rc__kf_boost,cpi__ppi__p_rc__rolling_actual_bits,cpi__common__show_frame,cpi__common__features__allow_high_precision_mv,cpi__twopass_frame__stats_in__coded_error,cpi__twopass_frame__stats_in__noise_var,cpi__twopass_frame__stats_in__log_intra_error");
#endif
